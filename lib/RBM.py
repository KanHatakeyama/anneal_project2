...
class RBM(nn.Module):
    ...
    def __init__(self,
        ...
        if not self.xavier_init:
            ...
            self.W = -self.xavier_value + \
                torch.rand(self.visible_units, self.hidden_units) * \
                (2 * self.xavier_value)
    ...
    def get_J(self):
        J = np.diag(list(self.v_bias.cpu().numpy()) +
                    list(self.h_bias.cpu().numpy()))
        W = self.W.cpu().numpy()
        W = np.pad(W, ((0, W.shape[1]), (W.shape[0], 0)))
        return W+J
    ...
    def reconstruct(self, X, n_gibbs=1):
        '''
    ...
    def contrastive_divergence(self, input_data, training=True,
        ...
        if(training):
            ...
            v_bias_update = torch.sum(
                input_data - negative_visible_probabilities, dim=0)/batch_size
            h_bias_update = torch.sum(
                positive_hidden_probabilities - negative_hidden_probabilities, dim=0)/batch_size
