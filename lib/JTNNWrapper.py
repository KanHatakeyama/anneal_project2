...
class JTNNWrapper:
    """
    Wrapper class of JTNN

    Attributes
    ----------
    model : object
        pretrained dgl model of JTNN
    """

    def __init__(self, model=None):
        if model is None:
            self.model = load_pretrained(
                "JTNN_ZINC")  # Pretrained model loaded
        else:
            self.model = model

        self.model.eval()
        self.path_smiles = "data/temp_smiles"

    def _write_smiles(self, smiles_list):
        """
        make file cantaining smiles list
    ...
    def _init_loader(self):
        ...
        dataset = JTVAEDataset(
            data=self.path_smiles, vocab=self.model.vocab, training=False
        )
    ...
    def encode(self, smiles_list):
        """
        encode smiles data (smiles to vectors)
        ...
        Returns
        -------
        np.array(v_list) : np array
            array of encoded data by JTNN
    ...
    def decode(self, vecs):
        """
        decode vector (vectors to smiles)
