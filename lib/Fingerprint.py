...
class Fingerprint:
    ...
    def __init__(self):
        #self.fp_func = self.morgan_fingerprint
        self.fp_func = lambda mol: fp_func(mol)
    ...
    def calc_fingerprint(self, SMILES_list, verbose=False):
        """
        ...
        """
        if verbose:
            fingerprint_list = [self.SMILES_to_bit(
                sm) for sm in tqdm(SMILES_list)]
        else:
