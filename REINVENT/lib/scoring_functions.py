...
import threading
import pexpect
from fp_func import fp_func
from multiprop_scoring_function import multiprop_scoring
...
def set_arbitary_bit(bit_string,fp_type="avalon"):
    mol = Chem.MolFromSmiles("C") 
    fp=fp_func(mol,fp_type=fp_type)    
    
    for position in range(len(bit_string)):
        flg=bool(int(bit_string[position]))
        
        if flg:
            fp.SetBit(position)
        else:
            fp.UnSetBit(position)
...
class fp_similarity():
    ...
    def __init__(self):
        self.query_fp=set_arbitary_bit(self.query_bit)
...
class fp_morgan_similarity():
    ...
    def __init__(self):
        self.query_fp=set_arbitary_bit(self.query_bit,fp_type="morgan")
        
    def __call__(self, smile):
        mol = Chem.MolFromSmiles(smile)
        if mol:
            fp=fp_func(mol,fp_type="morgan")
            score = DataStructs.TanimotoSimilarity(self.query_fp, fp)
            score = min(score, self.k) / self.k
            return float(score)
        return 0.0
    
class polym_fp_similarity():
    """Scores structures based on Tanimoto similarity to a query structure.
       Scores are only scaled up to k=(0,1), after which no more reward is given."""
    ...
    def __call__(self, smile):
        ...
        if mol:
            ...
            #number of Y should be 2
            score=score-abs(2-(smile.count("Y")))*0.5
...
def get_scoring_function(scoring_function, num_processes=None, **kwargs):
    ...
    scoring_function_classes = [no_sulphur, tanimoto, activity_model,multiprop_scoring,fp_similarity,polym_fp_similarity,fp_morgan_similarity]
