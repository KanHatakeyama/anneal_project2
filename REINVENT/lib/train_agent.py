...
def train_agent(restore_prior_from='data/Prior.ckpt',
    ...
    Prior = RNN(voc,device=device)
    Agent = RNN(voc,device=device)
    ...
    if torch.cuda.is_available():
        ...
        Prior.rnn.load_state_dict(torch.load('data/Prior.ckpt',map_location=device))
        Agent.rnn.load_state_dict(torch.load(restore_agent_from,map_location=device))
    ...
    for step in range(n_steps):
        ...
        unique_idxs = unique(seqs,device=device)
        seqs = seqs[unique_idxs]
        agent_likelihood = agent_likelihood[unique_idxs]
        entropy = entropy[unique_idxs]

        # Get prior likelihood and score
        prior_likelihood, _ = Prior.likelihood(Variable(seqs,device=device))
        smiles = seq_to_smiles(seqs, voc)
        score = scoring_function(smiles)

        # Calculate augmented likelihood
        augmented_likelihood = prior_likelihood + sigma * Variable(score,device=device)
        ...
        if experience_replay and len(experience)>4:
            ...
            exp_loss = torch.pow((Variable(exp_augmented_likelihood,device=device) - exp_agent_likelihood), 2)
    ...
    del Agent
    del Prior
    del seqs, agent_likelihood, entropy,unique_idxs
    del prior_likelihood,smiles,score,augmented_likelihood,loss
    del experience_replay,experience
    #del exp_agent_likelihood, exp_entropy,exp_augmented_likelihood,exp_loss
    del new_experience,loss_p,optimizer
...
def main():
    ...
    import json
    argvs={}
    argvs_cmd = json.loads(sys.argv[1])
    for key in argvs_cmd.keys():
        if key in argvs:
            argvs[key] = argvs_cmd[key] 
    return train_agent(**argvs)
