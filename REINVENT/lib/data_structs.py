...
class Vocabulary(object):
    ...
    def decode(self, matrix):
        ...
        smiles = "".join(chars)
        smiles = smiles.replace("L", "Cl").replace("R", "Br").replace("E", "Si")
        return smiles
...
def replace_halogen(string):
    """Regex to replace Br,Cl, and Si with single letters"""
    br = re.compile('Br')
    cl = re.compile('Cl')
    si = re.compile('Si')
...
def filter_mol(mol, max_heavy_atoms=30, min_heavy_atoms=3, 
               #element_list=[6,7,8,9,16,17,35]
               element_list=[5,6,7,8,9,14,15,16,17,35,39,53]
